...
This repository can be installed into the head node to facilatate rapid setup of an RNA-Seq analysis using `create-config.py`.
...
Note that supporting resources, including reference genomes, annotation files, and TruSeq adapters are not bundled with this repository.

## Slurm

Interactive Session:

`srun --pty -N 1 -p compute1 --nodelist compute1-st-compute-resource1-1  --exclusive bash`
